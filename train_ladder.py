...
model_config = "2_recon_1_SER_mod"
...
if is_stl:
    for attr in attr_list:
        E_model[attr] = E(6373, 256, 2, 10)
        D_model[attr] = D(6373, 256, 2, 10)
...
lm.alloc_stat_type_list(["train_emoclass", "train_recon",
    "dev_emoclass", "dev_all"])
if is_un:
...
dev_loader = DataLoader(dev_set, batch_size=20, shuffle=False)
...
for epoch in range(epochs):
    ...
    for x, y in train_loader:
        ...
        ce_stat={"emoclass":None}
        if is_stl:
            pred = torch.cat(pred, dim=1)
            # ccc = loss_manager.CCC_loss(pred, y)
            y = torch.flatten(y, start_dim=1)
            ser_loss = loss_manager.SER_CE_loss(pred, y)
            # loss = 1.0-ccc
            # total_loss += loss[0] + loss[1] + loss[2]
            total_loss+=1*ser_loss
            ce_stat["emoclass"] = ser_loss
            # ccc_stat["aro"] = ccc[0]
            # ccc_stat["dom"] = ccc[1]
            # ccc_stat["val"] = ccc[2]
        else:
        ...
        lm.add_torch_stat("train_emoclass", ce_stat["emoclass"])
    ...
    with torch.no_grad():
        # try:
            ...
            for x, y in dev_loader:
                ...
                pred = torch.cat(pred, dim=1)
                # print(y.shape, pred.shape, cur_pred, flush=True)
            ...
            # print(total_y.shape, total_pred.shape)
            total_y = torch.flatten(total_y, start_dim=1)
            ser_ce_loss = loss_manager.SER_CE_loss(total_pred, total_y)            
            lm.add_torch_stat("dev_emoclass", ser_ce_loss)
    ...
    lm.print_stat()
    for attr in attr_list:
        dev_loss = lm.get_stat("dev_"+str(attr))
...
print("Loss",end=" ")
for attr in attr_list:
    print(min_loss[attr], end=" ")
print("")
for attr in attr_list:
    for mtype in ["E", "D"]:
        os.system("cp "+os.path.join(model_path, "param", str(min_epoch[attr])+"_"+mtype+"_"+attr+".pt") + \
            " "+os.path.join(model_path, "final_"+mtype+"_"+attr+model_config+".pt"))    