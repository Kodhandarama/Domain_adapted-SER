...
test_path = os.path.join("data", "unlabeled", "Train")
model_config = "2_recon_1_SER_mod"
feat_path = "/home/eng/s/sxc220013/Documents/journal1/domain_adaptation_ser/speech-emotion-recognition-with-ladder-network/all_emotions.pkl"
...
if args.task_type == "STL":
    for attr in ["emoclass"]:
        E_model[attr] = E(6373, 256, 2, 10)
        E_model[attr].load_state_dict(torch.load(os.path.join(model_path, "final_E_"+attr+ model_config +".pt")))
...
if args.norm_type == 1:
    test_set = ds.HLDset(test_feats, None)
else:
...
if args.store_path is not None:
    ...
    with open(args.store_path, 'w') as f:
        for utt_id, p in zip(test_set.utt_list, total_pred):
            real_p = lab_std*(p.cpu().numpy()) + lab_mu
